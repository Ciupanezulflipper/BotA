...
APPROVAL="APPROVE BOTA SPLIT MANAGER RECONCILIATION DEPLOY AE204A40 EB53A5ED"
...
MANAGER_LOG="$ROOT/logs/runsvdir_manager.rollback.log"
...
for COMMAND in curl git python3 sv runsvdir; do
...
STAGE="$ROOT/audits/split_manager_deploy_${MAIN_COMMIT:0:8}_${MONO}"
...
if data.get("manager_count") != 1:
    reasons.append(f"manager_count:{data.get('manager_count')}")
if not data.get("manager_pid"):
    reasons.append("manager_pid_missing")
if data.get("owned") != 0:
    reasons.append(f"owned:{data.get('owned')}/7")
if data.get("orphaned") != 7:
    reasons.append(f"orphaned:{data.get('orphaned')}")
if data.get("running") != 7:
    reasons.append(f"running:{data.get('running')}/7")
...
for service in required:
    ...
    if row.get("service_running") is not True:
        reasons.append(f"service_down:{service}")
    if row.get("wrapper_alive") is not True:
        reasons.append(f"wrapper_dead:{service}")
...
print(
    ...
    f"MANAGER_PID={data.get('manager_pid')} "
...
if reasons:
    print(f"{label}_COMPATIBLE=NO")
    for reason in reasons:
        print(f"{label}_BLOCKER={reason}")
    raise SystemExit(7)
print(f"{label}_COMPATIBLE=YES")
print(f"{label}_MANAGER_PID={data['manager_pid']}")
PY
}

python3 "$STATUS" > "$RUNTIME/preflight.json" || true
validate_split "$RUNTIME/preflight.json" "PRE_MUTATION"
INITIAL_MANAGER="$(
...
launch_idle_manager() {
  if command -v setsid >/dev/null 2>&1; then
    nohup setsid "$PREFIX/bin/runsvdir" -P "$SERVICE_ROOT" \
      </dev/null >>"$MANAGER_LOG" 2>&1 &
  else
    nohup "$PREFIX/bin/runsvdir" -P "$SERVICE_ROOT" \
      </dev/null >>"$MANAGER_LOG" 2>&1 &
  fi
}

restore_split_topology() {
  local pid
  mapfile -t MANAGERS < <(standard_manager_pids)
  ...
  for _ in $(seq 1 45); do
    python3 "$STATUS" > "$RUNTIME/rollback_zero_manager.json" || true
    if python3 - "$RUNTIME/rollback_zero_manager.json" <<'PY'
import json
import sys
with open(sys.argv[1], encoding="utf-8") as handle:
    data = json.load(handle)
raise SystemExit(
    0 if (
        data.get("manager_count") == 0
        and data.get("orphaned") == 7
        and data.get("duplicate_service_rows") == 0
    ) else 1
)
PY
    then
  ...
  [ "$ZERO_READY" = YES ] || return 1

  launch_idle_manager

  SPLIT_READY=NO
  for _ in $(seq 1 45); do
    python3 "$STATUS" > "$RUNTIME/rollback_split.json" || true
    if validate_split "$RUNTIME/rollback_split.json" "ROLLBACK"
    then
      SPLIT_READY=YES
      break
    fi
...
rollback() {
  ...
  for SERVICE in \
    ...
    crond
  do
    sv -w 45 up "$SERVICE_ROOT/$SERVICE" \
      >/dev/null 2>&1 || true
  done

  if restore_split_topology; then
    printf 'AUTOMATIC_ROLLBACK=COMPLETE\n'
    printf 'RUNTIME_MUTATION_PERFORMED=ROLLED_BACK\n'
  else
    printf 'AUTOMATIC_ROLLBACK=INCOMPLETE\n'
    printf 'RUNTIME_MUTATION_PERFORMED=ROLLBACK_INCOMPLETE\n'
  fi
...
python3 "$STATUS" > "$RUNTIME/post_approval_pre_mutation.json" || true
validate_split \
  "$RUNTIME/post_approval_pre_mutation.json" \
  "POST_APPROVAL_PRE_MUTATION"

POST_APPROVAL_MANAGER="$(
  python3 - "$RUNTIME/post_approval_pre_mutation.json" <<'PY'
import json
import sys
with open(sys.argv[1], encoding="utf-8") as handle:
    print(json.load(handle)["manager_pid"])
...
[ "$POST_APPROVAL_MANAGER" = "$INITIAL_MANAGER" ] || {
  printf 'POST_APPROVAL_BLOCKER=MANAGER_PID_CHANGED:%s:%s\n' \
    "$INITIAL_MANAGER" "$POST_APPROVAL_MANAGER"
  false
}

mapfile -t POST_APPROVAL_GUARDS < <(guard_pids)
[ "${#POST_APPROVAL_GUARDS[@]}" -eq 0 ]
...
GUARD_PID=$GUARD_PID
INITIAL_IDLE_MANAGER_PID=$INITIAL_MANAGER
...
# IMMUTABLE_SPLIT_MANAGER_DEPLOY_END
