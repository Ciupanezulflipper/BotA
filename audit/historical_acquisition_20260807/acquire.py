...
def plan_requests(
    *,
    ...
    if max_candles < 2 or max_candles > MAX_CANDLES_PER_REQUEST:
    ...
    for instrument in instruments:
        if instrument not in DEFAULT_INSTRUMENTS:
            raise ValueError(f"unsupported audit instrument: {instrument}")
        for granularity in granularities:
            if granularity not in GRANULARITY_SECONDS:
                raise ValueError(f"unsupported audit granularity: {granularity}")
            seconds = GRANULARITY_SECONDS[granularity]
            span = timedelta(seconds=seconds * (max_candles - 1))
            cursor = start
            while cursor < end:
...
def build_preview(
    *,
    ...
    if host not in ALLOWED_HOSTS:
        raise ValueError("unapproved OANDA host")
...
def _response_metadata(response: TransportResponse) -> bytes:
    allowed_header_names = {"content-type", "date", "requestid", "request-id"}
    headers = {
        key: value
...
def parse_oanda_candles(body: bytes) -> list[dict]:
    ...
    for raw in raw_candles:
        ...
        timestamp = _utc(str(raw.get("time", "")))
    ...
    for chunk in chunks:
        for row in chunk:
            ...
            if prior is not None and prior != row:
                raise ValueError(f"conflicting duplicate candle timestamp: {key}")
            by_time[key] = row
...
def execute_acquisition(
    *,
    ...
    artifact_paths = [write_once(root, "plan/requests.json", plan_payload)]
    ...
    for index, request in enumerate(planned):
        ...
        response = fetch(host, request.path_and_query, headers, timeout_seconds)

        request_id = f"request-{index:04d}"
        raw_path = write_once(root, f"raw/{request_id}.json", response.body)
        req_path = write_once(
            root,
            f"metadata/{request_id}.request.json",
            _redacted_request_metadata(host, request),
        )
        resp_path = write_once(
            root,
            f"metadata/{request_id}.response.json",
            _response_metadata(response),
        )
        artifact_paths.extend([raw_path, req_path, resp_path])
    ...
    manifest_without_hash = {
        ...
        "artifacts": sorted(
            (_artifact_record(root, path) for path in artifact_paths),
    ...
    manifest_path = write_once(root, "manifest.json", manifest_body)
    return {
        "status": "PASS",
        "mode": "live_read_only",
        "run_id": run_id,
...
def main(argv: Iterable[str] | None = None) -> int:
    ...
    try:
        if args.command == "verify":
            result = verify_manifest(Path(args.output_root))
