...
def _event_metrics(outcome: dict[str, Any], event: dict[str, Any]) -> tuple[float, float]:
    ...
    published_entry = common.finite(outcome.get("entry_price"), "outcome entry_price")
    replay_entry = common.finite(event.get("entry", 0.0), "replay entry")
    if published_entry <= 0 or replay_entry <= 0:
        entry_pips = math.inf
    else:
        entry_pips = abs(published_entry - replay_entry) / common.pip_size(pair)
