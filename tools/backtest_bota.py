#!/usr/bin/env python3
"""
BotA Backtest Framework v2
==========================
Tests signal accuracy on OANDA historical M15 data.
Includes: BB, session quality, tick volume scoring.

Usage:
  python3 tools/backtest_bota.py                    # baseline
  python3 tools/backtest_bota.py --bb               # with Bollinger Bands
  python3 tools/backtest_bota.py --compare          # baseline vs BB
  python3 tools/backtest_bota.py --compare --days 60
"""
...
BB_PERIOD   = 20
BB_STD      = 2.0
SCORE_MIN   = float(os.environ.get("SCORE_MIN", "30"))
...
def fetch_oanda_candles(instrument: str, granularity: str, count: int = 500) -> List[Dict]:
    ...
    for _ in range(batches):
        ...
        params = f"?count=500&granularity={granularity}&price=MBA"
        if to_time:
            params += f"&to={to_time}"
        ...
        for c in candles:
            ...
            try:
                ...
                m = c["mid"]
                # Volume from bid candle (tick count)
...
def session_score(dt: datetime.datetime) -> float:
    """Session quality score based on candle timestamp."""
    if dt.tzinfo is None:
        dt = dt.replace(tzinfo=datetime.timezone.utc)
    h = dt.hour + dt.minute / 60.0
    if 12.0 <= h < 16.0:
        return 5.0   # London/NY overlap — peak quality
    elif 7.0 <= h < 12.0:
        return 2.0   # London only
    elif 16.0 <= h < 20.0:
        return 2.0   # NY only
    return 0.0       # Edge of session


def volume_score(candles: List[Dict], idx: int) -> float:
    """Tick volume confirmation vs 20-period average."""
    if idx < 20:
        return 0.0
    vols = [c["v"] for c in candles[idx-20:idx]]
    avg_vol = sum(vols) / len(vols) if vols else 0
    cur_vol = candles[idx]["v"]
    if avg_vol <= 0:
        return 0.0
...
def score_signal(candles: List[Dict], idx: int, use_bb: bool = False) -> Dict[str, Any]:
    ...
    if use_bb and len(closes) >= BB_PERIOD:
        ...
        if bu is not None and bl is not None:
            band_width_pct = ((bu - bl) / bm * 100.0) if bm else 0.0
    ...
    sess_bonus = session_score(candles[idx]["t"])

    # Volume component
    vol_bonus = volume_score(candles, idx)

    score = max(0, min(100, base_score + bb_bonus + sess_bonus + vol_bonus))
...
def run_backtest(candles: List[Dict], use_bb: bool, pair: str, days: int) -> Dict:
    ...
    for i in range(warmup, len(candles)):
        ...
        sig = score_signal(candles, i, use_bb=use_bb)
    ...
    if not signals:
        ...
        return {"error": f"no signals generated (in_session={in_sess}, candles={len(candles)}, warmup={warmup})"}
...
def print_report(result: Dict, label: str = ""):
    ...
    if result['wins'] > 0 and result['losses'] > 0:
        closed = result['signals']
        win_pips  = [s["pips"] for s in closed if s["outcome"] == "WIN"]
        loss_pips = [abs(s["pips"]) for s in closed if s["outcome"] == "LOSS"]
...
def main():
    ap = argparse.ArgumentParser(description="BotA Backtest Framework v2")
    ...
    if not OANDA_TOKEN:
        ...
        print("Run: source ~/BotA/.env && source ~/BotA/config/strategy.env && export OANDA_API_TOKEN OANDA_API_URL")
        sys.exit(1)
    ...
    fetch_count = min(int(args.days * 24 * 4 * 1.2), 5000)
